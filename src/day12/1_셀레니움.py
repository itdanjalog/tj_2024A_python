...
time.sleep(2) # 1초 일시정지(대기상태) # 웹페이지가 열릴때까지 2초 대기 # 컴퓨터 사양에 따라 다를수 있다

# 2. 커피빈 웹페이지의 자바스크립트 함수 호출 ,  .execute_script("JS함수호출")
wd.execute_script("storePop2(31)") # 31번 매장인 '삼성봉은사거리점' 모달 창 열린다.
time.sleep(2) # 1초 일시정지(대기상태) # 스크립트가 실행이 끝날때까지 2초 대기 # 컴퓨터 사양에 따라 다를수 있다
...
print( store_name ) # <h2>삼성봉은사거리점</h2>
