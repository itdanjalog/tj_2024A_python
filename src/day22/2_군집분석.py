...
# ### [2] 이를 통해 유효한 거래 데이터만 남기고, 오류나 비정상적인 데이터를 제거합니다. # 오류 데이터 정제
retail_df = retail_df[retail_df['Quantity'] > 0]    # 상품 수량이 0보다 크고
retail_df = retail_df[retail_df['UnitPrice'] > 0]   # 단가가 0보다 크며
retail_df = retail_df[retail_df['CustomerID'].notnull()] # 고객 ID가 결측되지 않은

# ### [3] 'CustomerID' 자료형을 정수형으로 변환
...
# ### [4] drop_duplicates() drop_duplicates()는 pandas의 메서드로, 중복된 행을 제거하는 기능을 수행합니다.  #중복 레코드 제거
...
# ### [8]  컬럼이름 바꾸기 #  열 이름을 변경하여 'InvoiceNo'을 'Freq'로, 'InvoiceDate'을 'ElapsedDays'로 변경합니다
...
# customer_df['ElapsedDays'] 열의 각 날짜에서 이 기준 날짜를 빼서 경과된 일수를 계산합니다
print( customer_df.head() ) #작업 확인용 출력
...
plt.scatter(customer_df['Freq'], customer_df['SaleAmount'], c=customer_df['Cluster'], cmap='viridis', marker='o', label='old data')
...
for i in range(6):
    ...
    plt.scatter(X_features_scaled[Y_labels == i, 0], X_features_scaled[Y_labels == i, 1],
                marker='o', color=color, edgecolor='black', s=50, label='cluster ' + str(i))
    # plt.scatter(kmeans.cluster_centers_[i, 0], kmeans.cluster_centers_[i, 1],
    #             marker='^', color=color, edgecolor='w', s=200)
...
# 2D 시각화
plt.figure(figsize=(10, 7))
plt.scatter(customer_df['Freq_log'], customer_df['SaleAmount_log'], c=customer_df['Cluster'], cmap='viridis', marker='o', label='old data')
