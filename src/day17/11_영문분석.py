...
plt.xticks(range(len(word_count)), list(sorted_Keys), rotation=85 , fontsize=6)
...
plt.xticks( range(len( summary_year) ) , [ text for text in summary_year['출판일'] ] , fontsize=6 )
# 차트 표시
plt.show()
...
wc = WordCloud( background_color='ivory').generate_from_frequencies( word_count )
plt.imshow( wc )
plt.show()
